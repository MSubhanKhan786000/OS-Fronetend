...
const Card = ({ image, name, rentPrice, buyPrice, description, status, userId, productId, rentStatus, buyStatus, onButtonClick }) => {
  ...
  return (
    <div className="card" style={{ width: '240px', height: '375px' }}>
      ...
      /> : <div><img src={image} alt="Card" className="card-image" />
...
const Search = (props) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/getCollection/cart/search/${search}`);
  ...
  console.log("This is+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++", itemsToShow)
